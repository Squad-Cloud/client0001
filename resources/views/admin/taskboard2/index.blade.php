...
<script>
    ...
    $('#save-form').click(function () {
        $.easyAjax({
            url: '{{route('admin.taskboard2.store')}}',
            container: '#addColumn',
    ...
    {{--$('#edit-column-form').on('click', '#update-form', function () {--}}
        ...
        {{--var url = '{{route('admin.taskboard2.update', ':id')}}';--}}
    ...
    function loadData () {
        ...
        var url = '{{route('admin.taskboard2.index')}}?startDate=' + startDate + '&endDate=' + endDate +'&clientID='+clientID +'&assignedBY='+ assignedBY+'&assignedTo='+ assignedTo+'&projectID='+ projectID+'&category_id='+ categoryId+'&label_id='+ labelId+'&milestone='+ milestone;
    ...
    $('body').on('click', '.edit-type', function () {
        var id = $(this).data('column-id');
        var url = '{{ route("admin.taskboard2.edit", ':id') }}';
        url = url.replace(':id', id);
