...
@section('page-title')
    <div class="row bg-title">
        ...
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 text-right bg-title-right">
            ...
            <a href="{{ route('admin.all-tasks2.create') }}" class="btn btn-outline btn-success btn-sm">@lang('modules.tasks.newTask') <i class="fa fa-plus" aria-hidden="true"></i></a>
...
{{--    @endif--}}
    ...
    <script>
        ...
        $('body').on('click', '.sa-params', function () {
            ...
            swal({
                ...
                if (isConfirm == 'confirm' || isConfirm == 'recurring') {
                    ...
                    var url = "{{ route('admin.all-tasks2.destroy',':id') }}";
        ...
        $('#allTasks-table').on('click', '.show-task-detail', function () {
            ...
            var id = $(this).data('task-id');
            var url = "{{ route('admin.all-tasks2.show',':id') }}";
            url = url.replace(':id', id);
        ...
        function exportData(){
            ...
            var url = '{!!  route('admin.all-tasks2.export', [':startDate', ':endDate', ':projectId', ':hideCompleted']) !!}';
        ...
        $('.pinnedItem').click(function(){
            var url = '{{ route('admin.all-tasks2.pinned-task')}}';
            $('#modelHeading').html('Pinned Task');
            $.ajaxModal('#taskCategoryModal',url);
        })
        $('#milestone').html("");
        function getMilestoneData(project_id){
                var url = "{{ route('admin.taskboard2.getMilestone', ':project_id') }}";
                var token = "{{ csrf_token() }}";
                $.easyAjax({
                url: url,
                type: "POST",
